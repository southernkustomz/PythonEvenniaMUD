...
class CmdDig(ObjManipCommand):
    ...
    def func(self):
        ...
        if to_exit["name"] == "north":
            new_room.x = cur_x
            new_room.y = cur_y + 1
            new_room.z = cur_z

        if to_exit["name"] == "south":
            new_room.x = cur_x
            new_room.y = cur_y - 1
            new_room.z = cur_z

        if to_exit["name"] == "east":
            new_room.x = cur_x + 1
            new_room.y = cur_y
            new_room.z = cur_z

        if to_exit["name"] == "west":
            new_room.x = cur_x - 1
            new_room.y = cur_y
            new_room.z = cur_z
        ...
        room_string = "Created room %s(%s)%s of type %s. Located at x:%s, y:%s, z:%s" % (
            new_room,
            new_room.dbref,
            alias_string,
            typeclass,
...
class CmdTunnel(COMMAND_DEFAULT_CLASS):
    ...
    This is a simple way to build using pre-defined directions:
     |wn,e,s,w|n (north, east, south, west)
     |wu,d|n (up and down)
     |wi,o|n (in and out)
    The full names (north, in, etc) will always be put as
